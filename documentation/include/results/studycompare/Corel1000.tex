...
The results presented in this section is \todo{are retrieved in accordance}in accordance with the evaluations described in Section \ref{sec:meth:eval:studycomp:corel}. Recall that the evaluation was \todo{is performed}run with $t=10$ categories, $m=500$ different query sets per category and number of retrieved images $n=20$.

The proposed model is adjusted to randomly sample the query data from the search space and makes sure that the query set and the test set are disjunct before an evaluation is performed. As mentioned in Section \ref{sec:meth:eval:studycomp} the number of images that the proposed model has in the training set was not decided on beforehand but was instead selected by inspecting the results of the similar studies. To do so the same evaluation was run with 6\todo{six} different traning\todo{query} set sizes: $2+2$, $3+3$, $4+4$, $5+5$, $7+7$ and $10+10$ relevant and irrelevant images. 
...
{fig:res:cbir:iictvc}
{0.9}
\todo{iictvc figure: final sentence. rephrase, ``changes logarithmicly''}

In both the Table \ref{table:res:cbir:prec20} and the Figure \ref{fig:res:cbir:iictvc} it \todo{it is notable that}becomes clear that the more query images the better the retrieval of the intended category becomes. This is somewhat in line with correlation that a separate image retrieval paper has found. The more query images \todo{images that were uses}they used the better results they\todo{results were recieved} received until a certain point on their \todo{dataset}data set; using \todo{six}6 images as a seed for \todo{for a performed search}their search resulted in a detection rate of $60\%$, with \todo{ten}10 images the rate turned out to be around $85\%$ \cite{li2010optimol}. In the paper detection rate is described as the ratio of relevant images that are perceived by the model as relevant. \todokahl{Konstig och ofullständig mening.}Very much like how \todo{average retrieval precision}precision is described in this evaluation and \todo{``the those''=> the}the those values are somewhere near the result given by the test of different query sizes\todo{``:''=> ``.''}: Using 3 relevant and 3 irrelevant images as query set resulted in a precision of $\approx53\%$ and using 5 relevant and 5 irrelevant images resulted in a precision of $\approx85\%$. \todo{Intriguingly when selecting a query set of 2 relevant... }What was interresting with selecting \todo{two}2 relevant and \todo{two}2 irrelevent images \todo{remove ``as query images''}as query images in a randomly generated manner was that in some cases the model could not find any categorial characteristics and therefor often found \todo{zero}0 relevant images during the retrieval. \todo{But as soon as a good...}Yet once a good query set was constructed the number of relevant images that was retrieved rose up to 15-20\todokahl{images. Formulera om meningen.}. 
...
Seeing that the proposed model performs this well compared to other CBIR methods is a good result. Yet it is still important to remember that having more query images reduces the risk of only having outliers as training\todo{query} data. \todo{The effect of this was}This was noticed more often when the query set size was set to $2+2$ and $3+3$, but considerably less when the query sets were larger than that. The other studies handles all images in the same category as an equal influence which gives the proposed model \todo{remove ``too much of ''}too much of an upper hand in this evaluation. \todo{But is still interesting since the proposed model}\todokahl{ofullständig mening}Which is interesting since the proposed model is designed to have much larger training/query sets in order to function as intended. 
