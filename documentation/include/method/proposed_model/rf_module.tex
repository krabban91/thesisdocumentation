...
There are a\todo{-a} numerous ways of using relevance feedback in order to improve CBIR and categorization\todo{categorizing} of material. In Section \ref{sec:theory:relfeed} the different ways of relevance feedback are divided into three categories and they are referred to as explicit, implicit and blind feedback. 
...
To present the \emph{top-k} images is a common method within CBIR, and the number of images that are presented could make it easier for the user to oversee the material. The number 20 was arbitrarily chosen and was \todo{empirically} manageable. An extension to this \emph{top-20} approach in order to improve classification was to also\todo{also to} present some images from the other extreme:\todo{a comma? ,} The \emph{bottom-k} images. With the intent to avoid drowning the user with information it sufficed with 5 images. Resulting in that the user could quickly give feedback to 25 images in total every iteration\todo{we mention later that we had a trail and error for different setups, is this in line with this text?}. 
